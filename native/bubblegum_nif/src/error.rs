...
impl From<BubblegumError> for rustler::Error {
    fn from(error: BubblegumError) -> Self {
        match error {
            BubblegumError::InvalidInput => rustler::Error::Term(Box::new("Invalid input".to_string())),
            BubblegumError::OperationFailed => rustler::Error::Term(Box::new("Operation failed".to_string())),
